...
|---|---|---|---|---|---|---|
| **Despliegue** | Cloudflare (conectado al repo de GitHub por su aplicación; sin credenciales en el repo) | D-BBW-10 | Panel de Cloudflare. El repo solo cumple el contrato `pnpm build` → `out/` (`DEPLOY_CONTRACT.md`) | ninguno en `src/` (el host sirve `out/`) | Cualquier host estático (Netlify, GitHub Pages, otro CDN) | Cambiar de panel y el registro DNS. **Cero cambios en el repo.** Estado: firmado; **previsualización por rama adelantada (D-BBW-26, fase 6f): pasos `[ZAVALA-MANUAL]` en `DEPLOY_CONTRACT.md` §7, sin dominio ni DNS; producción = cutover (fase 8)**. Local: `pnpm preview` (herramienta de desarrollo, no actor). |

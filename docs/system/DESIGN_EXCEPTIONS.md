...
**Retirada firmada (héroe rediagramado, D-BBW-47):** **EXC-BBW-02** (tamaño del rótulo `lead`, derivado del display por una sola
madre; firmada por D-BBW-19 el 2026-09-16 y ampliada por D-BBW-41 con la razón de interletrado invariante). **Se retira porque el rótulo
deja de existir**: el héroe pasa a un claim de tres líneas y el lockup de dos líneas que cerraban entre sí desaparece entero. Con la
excepción salen, en el mismo commit que entra su sustituto (I-6): las madres `--bbf-size-lead-ratio/-min/-max`, `--bbf-size-lead`,
`--bbf-tracking-lead-lock-ratio` y `--bbf-advance-text-max` en primitivos; los roles `--bbf-type-lead-*` en semantic; y el módulo
`src/behavior/optical-fit.ts` con el ajuste óptico y el peso por puntero que la consumían. **No se retira EXC-BBW-01** (el tamaño del
display), que es justo lo que el claim hereda, ni **EXC-BBW-04** (su interletrado), que sigue entrando en la guarda derivada y en la tabla
de avances del modulador. El ID 02 no se reutiliza. Fila retirada, conservada aquí por trazabilidad:

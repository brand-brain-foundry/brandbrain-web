...
async function main(): Promise<void> {
  ...
  const generated = [
    ...
    ' */',
    `/** Color de SUPERFICIE resuelto del rol \`${SURFACE_TOKEN}\` (D-BBW-75). Lo consume la etiqueta \`theme-color\` del documento, que`,
    ...
    `export const surfaceColor = ${JSON.stringify(surface.srgb)} as const; // COLOR-ALLOW: valor GENERADO resolviendo ${SURFACE_TOKEN}, no escrito a mano; su correspondencia con el token la vigila check-media.ts (huella de este archivo contra el lock, que sella el rol y su sRGB)`,
    '',
    'export const media = {',
    ...mediaEntries,
    '} as const;',

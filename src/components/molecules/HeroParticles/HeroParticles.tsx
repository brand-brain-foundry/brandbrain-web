 ...
 *   de los tokens (tamaño, desenfoque, opacidad, recorrido, duración) y a los tintes por rol.
...
const CLASS_FOR: Readonly<Record<ParticleKind, string>> = {
  ...
  moteFineSoft: `${styles.particle} ${styles.mote} ${styles.moteFineSoft}`,
...
function vars(p: Particle): CSSProperties {
  return { "--p-x": p.x, "--p-y": p.y, "--p-u": p.size, "--p-phase": p.phase, "--p-kf": p.k } as CSSProperties;
}

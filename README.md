...
- Tokens del sistema (madres + derivación por fórmula): `src/styles/tokens/` · fuentes: `src/styles/fonts/` · pesos por familia: [`docs/system/TYPOGRAPHY_WEIGHTS.md`](docs/system/TYPOGRAPHY_WEIGHTS.md)
- Contenido (fuera de los componentes, por locale publicado): `content/` → puerto `src/content/` · modelo: [`docs/system/CONTENT_MODEL.md`](docs/system/CONTENT_MODEL.md)
...
```bash
pnpm install
pnpm check   # typecheck + lint + guardias de color, tipografía y pesos por familia
